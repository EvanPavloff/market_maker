...
| Volatility kill-switch | Pause quoting if reference-price volatility spikes | **Not built.** `live_maker.py` reprices on drift but never *pauses* — a fast-moving market gets a fresh quote every cycle, not a halt. This is the single highest-value near-term addition; see `next_steps.md` #1. |
